...
async function asyncSortedMerge(logSources, printer) {
    ...
    const getNextLogs = () => Promise.all(
        logSources.map( (source) => source.popAsync())
    );

    let logs = await getNextLogs()

    for(const log of logs)
        if (log) minHeap.push(log);

    do {
        logs = await getNextLogs();
        ...
        for(const log of logs){
            if (log) {
                ...
                // we need the index to know which source the log entry came from
                minHeap.push(log);
                if(log.date.getTime() < min){
                    min = log.date;
                }
            }
        }

        while(minHeap?.top()?.date <= min){
            printer.print(minHeap.pop());
        }
    } while(logs.some(Boolean));
