 ...
 * @type {LogSource}
 * @typedef {{ date: Date, msg: string }} LogEntry
 */
module.exports = class LogSource {
   ...
   * @returns {LogEntry} A log entry object containing a date and a message.
   */
   ...
   * @returns {LogEntry | false} The next log entry or false if the log source is drained.
   */
  pop() {
   ...
   * If the date of the next log entry is in the future, the log source is marked as drained.
   * @returns {Promise<LogEntry | false>} A promise that resolves with the next log entry or false if the log source is drained.
   */
  popAsync() {
